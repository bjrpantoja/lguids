  ...
  while (terminate == 0) /* endless loop */
  {
    ...
    while (!terminate && DEVICE.outgoing)
    {
      ...
      if (i > 0)
      {
        ...
        if (dspooler_check_max_continuous_sending(started_sending, continuous_sent))
        {
          quick = 0;
          workless = 0;
          break;
    ...
    if (workless == 1) // wait a little bit if there was no SM to send or receive to save CPU usage
    {
      ...
      if (!spend_delay(delaytime, &dspooler_run_rr, &last_rr, DEVICE.dev_rr_interval))
        return;
    }
...
void abnormal_termination(int all)
{
  ...
  if (process_id == -1)
  {
    ...
    if (mainprocess_notifier_pid)
      kill(mainprocess_notifier_pid, SIGTERM);
