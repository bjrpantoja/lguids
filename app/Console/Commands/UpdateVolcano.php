...
class UpdateVolcano extends Command
{
     ...
     * @var string
     */
    protected $description = 'Fetch volcano update to PHIVOLCS website.';
    ...
    {
        $url                = Setting::pluck('st_address').'/updates/volcano.txt';
        $message            = file_get_contents($url);

        if(!empty($message)) {
            
            $latest_bulletin    = Bulletin::where('bl_message', '=', $message)->where('bt_id', '=', '2')->latest('bl_id')->get();
